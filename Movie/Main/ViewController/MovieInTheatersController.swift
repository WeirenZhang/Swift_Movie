...
class MovieInTheatersController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ApiLoadingProvider.request(Api.movie_intheaters(page:page)) {
            result in
    ...
    @objc private func loadData(more: Bool) {
        ...
        ApiProvider.request(Api.movie_intheaters(page:page)) {
            result in
            var message = "Couldn't access API"
            if case let .success(response) = result {
                let jsonString = try? response.mapString()
...
extension MovieInTheatersController: UITableViewDelegate, UITableViewDataSource {
