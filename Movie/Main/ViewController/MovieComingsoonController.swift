...
class MovieComingsoonController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ApiLoadingProvider.request(Api.movie_comingsoon(page:page)) {
            result in
    ...
    @objc private func loadData(more: Bool) {
        ...
        ApiProvider.request(Api.movie_comingsoon(page:page)) {
            result in
            var message = "Couldn't access API"
            if case let .success(response) = result {
                let jsonString = try? response.mapString()
...
extension MovieComingsoonController: UITableViewDelegate, UITableViewDataSource {
